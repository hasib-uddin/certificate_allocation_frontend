...
export default function AddBulkRecipient({
  recipientOpen,
  setRecipientOpen
}) {
  ...
  const submithandleBulkRecipient = async () => {
    ...
    if (!selectedFile) {
      setLoading(false);
      setNetworkError("Please select a file.");
      return;
    }
    const formData = new FormData();
    formData.append("excelFile", selectedFile);
    const body = formData
  console.log(selectedFile,formData,"51")

    try {

      const response = await fetch(`/recipients/add_bulk_recipient`, "post", body);
